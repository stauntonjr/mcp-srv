...
export class UpstreamManager extends EventEmitter {
    ...
    constructor(capabilityRegistry, maxRetries = 3, retryDelay = 2000) {
        super();
   ...
   */
     ...
     */
    async updateConnections(newConfig) {
        ...
        const successfulAdds = addResults.filter(result => result.status === 'fulfilled' && result.value.success).length;
     ...
     */
    async addServerWithTiming(name, config) {
    ...
    async shutdown() {
        ...
        for (const name of this.getServerNames()) {
            disconnectPromises.push(this.disconnectServer(name).catch(error => {
    ...
    createConnector(name, config) {
        ...
        switch (serverType) {
            case 'stdio':
                return new StdioConnector(name, config);
            case 'sse':
            ...
            case 'streamable-http':
                ...
                return new StreamableHttpConnector(name, config);
            default:
     ...
     */
    setupConnectorHandlers(name, connector) {
     ...
     */
    async discoverServerCapabilities(name) {
        const connector = this.connectors.get(name);
        const connection = this.connections.get(name);
        if (!connector || !connection) {
            return;
     ...
     */
    async routeMessage(serverName, message) {
...
//# sourceMappingURL=UpstreamManager.js.map